...
function Edit() {
  ...
  const handleOnWheel = (e) => {
    if (e.ctrlKey) {
      e.preventDefault();
      let currentScale = parseFloat(canvaRef.current.style.transform.match(/\d+\.\d+/))
  ...
  useEffect(() => {
    canvaContainerRef.current.addEventListener('wheel', handleOnWheel, { passive: false});
    canvaRef.current.style.transform = 'scale(0.75)';
  }, [])

  return (
    <div className="w-full h-full flex flex-row ">
      <div ref={canvaContainerRef} className="w-full h-full bg-[#f1f3f4] mr-[260px] pt-[75px]">
        <div
          ref={canvaRef}
